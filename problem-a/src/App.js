...
export class SenatorTable extends Component {
  render() {
    let senatorRowArr = this.props.senators.map((senator) => <SenatorRow key={senator.name} senator={senator} />)
    return (
...
export class TableHeader extends Component {
  render() {
    let colNameArr = this.props.cols.map((colName) => <th key={colName}>{colName}</th>)
    return (
      <thead>
        <tr>
          {colNameArr}
